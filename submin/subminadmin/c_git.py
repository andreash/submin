...
Usage:
    ...
    def subcmd_init(self, args):
        ...
        ssh_port = self.prompt_user("On what port is the ssh-server available internally?",
                "22")
    ...
    def create_user_adduser(self, username, homedir):
        self.delete_home_directory(homedir)
        cmd = ["adduser",
               "--system",
               "--shell /bin/sh",
    ...
    def delete_home_directory(self,homedir):
    ...
    def delete_user(self,username):
        ...
        cmd = ' '.join(cmd)
        executeCmd(cmd,"Could not delete existing user %s" % username)
